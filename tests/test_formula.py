...
from vcfstats.formula import parse_subsets, Term, Aggr, One, Formula
...
def test_aggr_init():
    ...
    assert repr(aggr) == "<Aggr COUNT(<Term _ONE()>, filter=<Term FILTER(subsets=['PASS'])>, group=<Term VARTYPE()>)>"
    assert aggr.has_filter()
...
def test_aggr_run(variants):
    ...
    aggr3 = Aggr('COUNT', One(), filter=Term('FILTER', ['PASS']), group=Term('FILTER2'))
...
def test_formula_run(variants):
    ...
    datafile.truncate(0)
    datafile.seek(0)
    fmula = Formula('COUNT(1, filter=VARTYPE) ~ COUNT(1, filter=GTTYPEs, group=CHROM)', variants[-1], False, 'title')
    fmula.run(variants[0], datafile)
    ...
    with pytest.raises(ValueError):
        Formula('COUNT(1, filter=VARTYPE, group=GTTYPEs{0}) ~ COUNT(1, filter=GTTYPEs, group=CHROM)', ['A', 'B', 'C', 'D'], False, 'title')
